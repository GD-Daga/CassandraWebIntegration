...
import { faXmark, faChevronDown, faCircleXmark } from "@fortawesome/free-solid-svg-icons";
...
const CatholicChurches: React.FC = () => {
  ...
  const handleChurchSelect = (churchName: string) => {
    ...
    navigate(
      `/district-map?district=${encodeURIComponent(selectedDistrict || "")}&church=${encodeURIComponent(churchName)}`
    );
  };
  ...
  const handleAddBookmark = async (church: any) => {
    ...
    try {
      ...
      setToastMessage({ type: "success", text: `${church.name} has been successfully added to your bookmarks!` });
    } catch (error) {
  ...
  return (
    <div>
      {/* Main Card */}
      <div
        className="relative flex items-center justify-center flex-col cursor-pointer transform transition-transform duration-300 ease-in-out hover:scale-100 border border-gray-300 rounded-lg shadow-md"
        onClick={() => setShowDistrictModal(true)}
      >
        <img
          className="w-full h-[200px] object-cover rounded-t-lg transition-transform duration-300 ease-in-out hover:scale-105"
      ...
      {showDistrictModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-8 rounded-lg w-[90%] md:w-[50%] relative max-h-[80vh] overflow-y-auto border border-gray-300 shadow-lg" ref={districtModalRef}>
            <h3 className="text-xl font-bold mb-4 sticky top-0 bg-white z-10 border-b">Select a District</h3>
            <button className="mb-2 text-blue-600 hover:underline hover:text-blue-800" onClick={handleViewBookmarks}>
            ...
            {toastMessage && (
              <div
                className={`absolute top-2 right-2 z-50 p-2 rounded shadow-lg text-white text-sm flex items-center justify-between ${
            ...
            <ul className="space-y-4">
              {districts.map((district, idx) => (
                <li key={idx} className="border-b pb-2">
                  <div
                    className="flex items-center justify-between cursor-pointer text-blue-500 hover:underline"
                    onClick={() => handleDistrictToggle(district)}
                  >
                    <span>{district}</span>
                    <FontAwesomeIcon
                      icon={faChevronDown}
                      className={`transition-transform duration-300 ${openDistrict === district ? "rotate-180" : ""}`}
                    />
                  </div>
                      ...
                      {getChurchesByDistrict(district).map((church) => (
                        <li key={church.name} className="mb-2">
                          <div className="flex justify-between items-center">
                            <span className="cursor-pointer hover:underline" onClick={() => handleChurchSelect(church.name)}>
                              {church.name}
                            </span>
                            <button
                              className="text-sm px-4 py-2 text-white bg-[#6b2e2e] rounded hover:bg-[#634e27]"
            ...
            <button className="absolute top-2 right-2 text-gray-500 hover:text-gray-800" onClick={() => setShowDistrictModal(false)}>
...
{showBookmarksModal && (
  <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
    <div
      className="bg-white p-8 rounded-lg w-[90%] md:w-[50%] relative max-h-[80vh] overflow-y-auto border border-gray-300 shadow-lg"
      ref={bookmarksModalRef}
    >
      <h3 className="text-xl font-bold mb-4 sticky top-0 left-0 right-0 bg-white z-10 border-b py-4 px-4">
        Bookmarked Churches
      </h3>
      {isLoading ? (
          ...
          {bookmarks.length > 0 ? (
            bookmarks.map((bookmark, idx) => (
              <li key={idx} className="flex justify-between items-center border-b py-2">
                <span className="text-blue-600 hover:underline">{bookmark.church_name}</span> - {bookmark.address}
                <div className="flex space-x-2">
                  ...
                  <button
                    className="text-white px-4 text-sm bg-red-500 rounded hover:bg-red-700"
      ...
      <button className="absolute top-2 right-2 text-gray-500 hover:text-gray-800" onClick={() => setShowBookmarksModal(false)}>
        <FontAwesomeIcon icon={faXmark} />
      </button>
    </div>
  </div>
